...
const Notifications = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await fetchNotificationNumber(token);
        await fetchProjects(token);
        await fetchUser(token);
        await markNotificationsChecked(token);
      } catch (error) {
  ...
  const fetchNotificationNumber = async (token) => {
    try {
  ...
  const markNotificationsChecked = async (token) => {
    try {
      await axios.put('http://localhost:8081/api/project/notificationChecked', {}, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
    } catch (error) {
      console.error('Error marking notifications checked:', error);
    }
  };
  ...
  return (
    <div className="container">
      ...
      <div className="content">
        ...
        <div className="tabs">
                <div
                    className={`tab ${activePanel === 'ActiveNotifications' ? 'active-tab' : ''}`}
                    onClick={() => switchPanel('ActiveNotifications')}
                >
                    Notices
                </div>
                <div
                    className={`tab ${activePanel === 'Invitations' ? 'active-tab' : ''}`}
                    onClick={() => switchPanel('Invitations')}
                >
                    Project Invitations
                </div>
            </div>
            <div className="component-container">
                {activePanel === 'ActiveNotifications' && (
                    <ActiveNotifications />
                )}
